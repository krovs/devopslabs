    ...
    blocks: [
      {
        type: "paragraph",
        content: [
          "Incident response coordinates detection, command, triage, communication, and postmortem. The goal is to restore service and learn without blame.",
        ],
      },
      { type: "heading", text: "Incident Roles" },
      {
        type: "unorderedList",
        items: [
          ["Incident Commander: leads the response, names severity, delegates actions."],
          ["Operations Lead: investigates and mitigates the technical cause."],
          ["Communications Lead: drafts and publishes status updates."],
        ],
      },
      { type: "heading", text: "SEV Levels" },
      {
        type: "unorderedList",
        items: [
          ["SEV1: critical user impact, requires immediate commander assignment."],
          ["SEV2: significant impact, commander optional but recommended."],
          ["SEV3: minor impact, can be handled by on-call without escalation."],
        ],
      },
      { type: "heading", text: "Postmortem Structure" },
      {
        type: "orderedList",
        items: [
          ["Symptom: what did the customer see and for how long?"],
          ["Root Cause: the technical failure that triggered the incident."],
          ["Detection: how was the incident found (alert, customer report, etc.)?"],
          ["Action Items: concrete, owner-assigned tasks that prevent recurrence."],
      ...
      { type: "heading", text: "Alert Triage" },
      {
        type: "unorderedList",
        items: [
          ["Identify the root alert by correlating timing and dependency graphs."],
          ["Suppress downstream symptom alerts so the responder can focus on cause."],
          ["Status updates follow a regular cadence: t+0 acknowledge, t+15 investigating, t+60 mitigated."],
        ],
      },
    ],
  },
  {
    id: "wiki-disaster-recovery",
    ...
    blocks: [
      {
        type: "paragraph",
        content: [
          "Disaster recovery is the ability to restore service after a regional or service-level failure. DR planning defines RPO/RTO, exercises failover, and verifies backups.",
        ],
      },
      { type: "heading", text: "Key Metrics" },
      {
        type: "unorderedList",
        items: [
          [{ code: "RPO (Recovery Point Objective)" }, ": maximum acceptable data loss measured in time."],
          [{ code: "RTO (Recovery Time Objective)" }, ": maximum acceptable downtime measured in time."],
          ["RPO is bounded by backup/replication cadence. RTO is failover duration plus verification."],
        ],
      },
      { type: "heading", text: "Common Patterns" },
      {
        type: "unorderedList",
        items: [
          ["Multi-AZ RDS: promote standby, verify app reconnects before cutover."],
          ["Region failover: Route 53 weighted routing shifts traffic to DR region."],
          ["Backup integrity: restore to staging and diff checksums/row counts against prod."],
    ...
    blocks: [
      {
        type: "paragraph",
        content: [
          "Database operations cover failover, replication, query performance, connection management, and point-in-time recovery. Diagnose state before applying destructive changes.",
        ],
      },
      { type: "heading", text: "RDS Operations" },
      {
        type: "unorderedList",
        items: [
          ["Multi-AZ failover: check event log, standby health, and DNS before forcing."],
          ["Read replica lag: kill long-running transactions on the primary and clear stalled checkpoints."],
          ["Query regression: use EXPLAIN to detect plan flips; add indexes on filtered columns."],
          ["PITR: restore to a specific timestamp; verify the target is inside the retention window."],
        ],
      },
      { type: "heading", text: "Connection Pooling" },
      {
        type: "unorderedList",
        items: [
          [{ code: "session" }, " mode pins one backend per client; use ", { code: "transaction" }, " mode for bursty apps."],
          ["Leaky clients holding idle connections exhaust the pool. Identify and fix them first, then raise maxClientConn."],
        ],
      },
      { type: "heading", text: "DynamoDB" },
      {
        type: "unorderedList",
        items: [
          ["Hot key: one partition absorbs most writes. Enable key sharding or suffix-based distribution."],
    ...
    blocks: [
      {
        type: "paragraph",
        content: [
          "Supply chain security protects the software delivery pipeline from source to artifact. It covers SBOM generation, artifact signing, provenance attestation, vulnerability gates, and dependency integrity.",
        ],
      },
      { type: "heading", text: "SBOM" },
      {
        type: "unorderedList",
        items: [
          ["CycloneDX is the standard format for machine-readable software bills of materials."],
          ["Release gates require an SBOM before deployment to provide verifiable inventory."],
        ],
      },
      { type: "heading", text: "Artifact Signing" },
      {
        type: "unorderedList",
        items: [
          ["Cosign signs container images with a keyless or keyed identity."],
          ["Kubernetes admission controllers reject unsigned images when verify is enabled."],
        ],
      },
      { type: "heading", text: "SLSA" },
      {
        type: "unorderedList",
        items: [
          ["SLSA (Supply-chain Levels for Software Artifacts) defines 4 levels of build integrity."],
          ["Level 3 requires a provenance attestation with a verified builder identity."],
          ["The builder ID identifies the build platform (e.g. github-actions) and slsa-verifier validates attestations."],
        ],
      },
      { type: "heading", text: "Dependency Confusion" },
      {
        type: "unorderedList",
        items: [
          ["Private package names can be registered on public registries."],
    ...
    blocks: [
      {
        type: "paragraph",
        content: [
          "SRE uses service level objectives to balance reliability with feature velocity. An SLI measures a user-facing signal; an SLO sets a reliability target; an error budget governs how much unreliability is acceptable before freezing changes.",
        ],
      },
      { type: "heading", text: "SLI Formulation" },
      {
        type: "unorderedList",
        items: [
          [{ code: "SLI = good events / total events" }, " over a rolling window."],
          ["Good events: HTTP requests that do not return a 5xx status code."],
          ["Total events: every HTTP request to the service."],
        ],
      },
      { type: "heading", text: "Error Budgets" },
      {
        type: "unorderedList",
        items: [
          ["Error budget = 1 - SLO. For 99.9% SLO, the budget is 0.1%."],
          ["Burn rate: how fast the error budget is consumed. A 1x burn rate consumes the 30-day budget in 30 days."],
          ["Multi-window burn alert: a short window (5m) catches fast burns, a long window (1h) catches slow burns."],
        ],
      },
      { type: "heading", text: "SLO Tiers" },
      {
        type: "unorderedList",
        items: [
          ["Tier 1: critical user-facing services. SLO typically 99.95%+."],
          ["Tier 2: important but not critical. SLO typically 99.9%."],
          ["Tier 3: internal tools. SLO typically 99.0% with lenient budget policy."],
        ],
      },
      { type: "heading", text: "Toil" },
      {
        type: "unorderedList",
        items: [
          ["Toil is manual, repetitive, automatable operational work."],
          ["SRE teams should keep toil under 50% of sprint time."],
    ...
    blocks: [
      {
        type: "paragraph",
        content: [
          "Messaging infrastructure handles asynchronous communication between services. Topics, queues, and streams provide reliable delivery with patterns for redrive, filtering, ordering, and idempotency.",
        ],
      },
      { type: "heading", text: "SQS DLQ Redrive" },
      {
        type: "unorderedList",
        items: [
          ["Dead Letter Queue catches messages that exceed the max receive count."],
          ["Redrive policy links a source queue to its DLQ so poison messages are moved instead of retried."],
          ["Set maxReceiveCount to a bounded number (e.g. 5) before the DLQ receives the message."],
          ["Add idempotency checks in the consumer so redelivered messages are not double-processed."],
        ],
      },
      { type: "heading", text: "Kafka Consumer Rebalance" },
      {
        type: "unorderedList",
        items: [
          ["Rebalance storms happen when consumers drop out of the session window or partitions reshuffle."],
          ["Raise session.timeout.ms to tolerate transient pauses (e.g. 45s)."],
          ["Use static group membership (group.instance.id) to avoid reshuffle on transient disconnects."],
          ["Switch to cooperative-sticky assignment to rebalance incrementally instead of stopping all consumers."],
        ],
      },
      { type: "heading", text: "SNS Filter Policy" },
      {
        type: "unorderedList",
        items: [
          ["Fan-out sends every message to every subscriber by default."],
          ["FilterPolicy restricts a subscription to matching message attributes."],
          ["Example: subscription only receives messages where event = order.created."],
        ],
      },
      { type: "heading", text: "Ordering and Idempotency" },
      {
        type: "unorderedList",
        items: [
          ["Ordered delivery: use a stable partition key (e.g. orderId) so events for one entity stay on one partition."],
          ["Hot partition: when one key absorbs most writes. Use key sharding to spread writes."],
...
const referenceSectionByKind: Record<NonNullable<Scenario["kind"]>, string> = {
  ...
  incident: "wiki-incident-response",
  dr: "wiki-disaster-recovery",
  database: "wiki-database-ops",
  supplychain: "wiki-supply-chain",
  sre: "wiki-sre-slo",
